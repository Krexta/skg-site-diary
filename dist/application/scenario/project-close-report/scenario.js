...
import { ProjectCloseReportParser } from "./parser.js";
export class ProjectCloseReportScenario {
    async pushProjectCloseReport(input) {
        try {
    ...
    async update(id, updateData) {
        try {
            const entity = await this.repository.getProjectCloseReportById(ProjectReportId.from(id));
            const data = this.parser.formatDataToUpdate(entity.createUserId.value, updateData);
            const updatedEntity = entity.update(data);
            const result = await this.repository.updateProjectCloseReport(updatedEntity);
            return this.parser.convertProjectReportToReference(result);
        } catch (error) {
            if ([
                BadRequestException,
...
ProjectCloseReportScenario = _ts_decorate([
    Injectable(),
