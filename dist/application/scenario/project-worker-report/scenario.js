...
import { ProjectWorkerReportParser } from "./parser.js";
export class ProjectWorkerReportScenario {
    async pushProjectWorkerReport(input) {
        try {
            const entity = this.parser.convertToProjectReport(input);
            const result = await this.repository.createProjectWorkerReport(entity);
    ...
    async update(id, updateData) {
        try {
            const entity = await this.repository.getProjectWorkerReportById(ProjectReportId.from(id));
            const data = this.parser.formatDataToUpdate(entity.createUserId.value, updateData);
            const updatedEntity = entity.update(data);
            const result = await this.repository.updateProjectWorkerReport(updatedEntity);
            return this.parser.convertProjectReportToReference(result);
        } catch (error) {
            if ([
                BadRequestException,
    ...
    constructor(logger, repository){
        ...
        this.logger.setContext(ProjectWorkerReportScenario.name);
        this.parser = new ProjectWorkerReportParser(this.logger);
    }
}
ProjectWorkerReportScenario = _ts_decorate([
    Injectable(),
