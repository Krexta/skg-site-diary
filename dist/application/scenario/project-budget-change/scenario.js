...
import { ProjectBudgetChangeParser } from "./parser.js";
export class ProjectBudgetChangeScenario {
    async pushProjectBudgetChange(input) {
        try {
    ...
    async update(id, updateData) {
        try {
            const entity = await this.repository.getProjectBudgetChangeById(ProjectReportId.from(id));
            const data = this.parser.formatDataToUpdate(entity.createUserId.value, updateData);
            const updatedEntity = entity.update(data);
            const result = await this.repository.updateProjectBudgetChange(updatedEntity);
            return this.parser.convertProjectBudgetChangeToReference(result);
        } catch (error) {
            if ([
                BadRequestException,
...
ProjectBudgetChangeScenario = _ts_decorate([
    Injectable(),
