...
import { ProgressCheckReportParser } from "./parser.js";
export class ProgressCheckReportScenario {
    async pushProgressCheckReport(input) {
        try {
            const entity = this.parser.convertToProjectReport(input);
            const result = await this.repository.createProgressCheckReport(entity);
    ...
    async update(id, updateData) {
        try {
            const entity = await this.repository.getProgressCheckReportById(ProjectReportId.from(id));
            const data = this.parser.formatDataToUpdate(entity.createUserId.value, updateData);
            const updatedEntity = entity.update(data);
            const result = await this.repository.updateProgressCheckReport(updatedEntity);
            return this.parser.convertProjectReportToReference(result);
        } catch (error) {
            if ([
                BadRequestException,
    ...
    constructor(logger, repository){
        ...
        this.logger.setContext(ProgressCheckReportScenario.name);
        this.parser = new ProgressCheckReportParser(this.logger);
    }
}
ProgressCheckReportScenario = _ts_decorate([
    Injectable(),
