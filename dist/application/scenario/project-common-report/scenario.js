...
import { ProjectCommonReportParser } from "./parser.js";
export class ProjectCommonReportScenario {
    async pushProjectCommonReport(input) {
        try {
            const entity = this.parser.convertToProjectReport(input);
            const result = await this.repository.createProjectCommonReport(entity);
    ...
    async update(id, updateData) {
        try {
            const entity = await this.repository.getProjectCommonReportById(ProjectReportId.from(id));
            const data = this.parser.formatDataToUpdate(entity.createUserId.value, updateData);
            const updatedEntity = entity.update(data);
            const result = await this.repository.updateProjectCommonReport(updatedEntity);
            return this.parser.convertProjectReportToReference(result);
        } catch (error) {
            if ([
                BadRequestException,
    ...
    constructor(logger, repository){
        ...
        this.logger.setContext(ProjectCommonReportScenario.name);
        this.parser = new ProjectCommonReportParser(this.logger);
    }
}
ProjectCommonReportScenario = _ts_decorate([
    Injectable(),
