...
import { ProjectMeetingReportParser } from "./parser.js";
export class ProjectMeetingReportScenario {
    async pushProjectMeetingReport(input) {
        try {
            const entity = this.parser.convertToProjectReport(input);
            const result = await this.repository.createProjectMeetingReport(entity);
    ...
    async update(id, updateData) {
        try {
            const entity = await this.repository.getProjectMeetingReportById(ProjectReportId.from(id));
            const data = this.parser.formatDataToUpdate(entity.createUserId.value, updateData);
            const updatedEntity = entity.update(data);
            const result = await this.repository.updateProjectMeetingReport(updatedEntity);
            return this.parser.convertProjectReportToReference(result);
        } catch (error) {
            if ([
                BadRequestException,
    ...
    constructor(logger, repository){
        ...
        this.logger.setContext(ProjectMeetingReportScenario.name);
        this.parser = new ProjectMeetingReportParser(this.logger);
    }
}
ProjectMeetingReportScenario = _ts_decorate([
    Injectable(),
